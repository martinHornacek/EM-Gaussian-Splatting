...
We retain the signed rendering variant~\eqref{eq:signed_corr} as the
ablation baseline and implement coverage correction as a natural avenue
for future work.
...
$K_\mathrm{tile}\!=\!16$ the per-tile residual refinement has too few
components to correct effectively, degrading quality by the same
overshoot mechanism analysed in Section~\ref{sec:hybrid}.
...
\hline
\multicolumn{4}{l}{\footnotesize $^\dagger$Per-tile residual refinement degrades at $K_{\mathrm{tile}}\!=\!16$; see Section~\ref{sec:hybrid}.}\\
\hline
\end{tabular}
\end{table}
