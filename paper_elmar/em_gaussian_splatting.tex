...
The refinement consistently \emph{reduces} quality by 0.9--2.8\,dB
...
2048 & 23.64$^\dagger$ & 22.71 & $-$0.93 \\
\hline
\multicolumn{4}{l}{\footnotesize $^\dagger$Mini-batch EM baseline (full-batch run incomplete).}\\
\hline
\end{tabular}
\end{table}

Extending the ablation to $K\!=\!2048$ confirms the trend: the hybrid
achieves 22.71\,dB versus 23.64\,dB for mini-batch EM (Table~\ref{tab:minibatch}),
a gap of $-$0.93\,dB.
...
The mini-batch variant also extends naturally to $K\!=\!2048$ (23.64\,dB,
193\,s per image), a regime where full-batch fitting becomes prohibitively slow.
...
where full-batch fitting is impractical.
...
from $-$2.79\,dB at $K\!=\!128$ to $-$0.93\,dB at $K\!=\!2048$.
...
Combining EM initialisation with gradient-based fine-tuning and
a full comparison against gradient-based 2D Gaussian Splatting
